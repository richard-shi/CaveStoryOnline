...
// Main loop globals
static bool fresh_start = true;
static bool inhibit_loadfade = false;

// Game loop globals
static int32_t nexttick = 0;
  ...
  if (game.switchstage.mapno == LOAD_GAME || \
          ...
          game.switchstage.eventonentry == 91)
        {
          fresh_start = true;
			...
			// switch maps
			if (load_stage(game.switchstage.mapno)) ingame_error();
